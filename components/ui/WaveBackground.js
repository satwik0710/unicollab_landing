function WaveBackground({
    ...
    const boundingRef = React.useRef(null);

    // Initialization
    React.useEffect(() => {
        if (!containerRef.current || !svgRef.current) return;

        // Initialize noise generator using global SimplexNoise (from script tag)
        if (window.SimplexNoise) {
            const simplex = new SimplexNoise();
            noiseRef.current = (x, y) => simplex.noise2D(x, y);
        } else {
            console.warn("SimplexNoise library not loaded");
            // Fallback noise function
            noiseRef.current = (x, y) => Math.sin(x) * Math.cos(y);
        }
